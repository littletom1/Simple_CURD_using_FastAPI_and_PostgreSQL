...
# from cores.mongodb.database import database, client
...
class ServiceBase(ServiceBaseContract):
    ...
    async def paginate(self, pagination_params, query:dict = {}, with_trash: bool = False, sort_by:dict = None, direction = None):
    ...
    async def search(self, name:str, is_absolute:bool = True, is_get_first:bool = True):
        q_collection = self.collection
        if is_get_first:
            if is_absolute:
                return self.convert_data(await q_collection.find_one({'name': name}))
            else:
                async for obj in q_collection.find({"name": {"$regex": name, "$options": "i"}}):
                    return self.convert_data(obj)
        else:
            if is_absolute:
                objs = []
                async for obj in q_collection.find({'name': name}):
                    objs.append(self.convert_data(obj))
                return objs
            else:
                objs = []
    ...
    async def update(self, id:str, data: object) -> dict:
        ...
        if q_obj.find_one({'_id': ObjectId(id)}):
    ...
    async def delete(self, id:str, is_hard_delete = False) -> dict:
        q_obj = self.collection
        if await q_obj.find_one({'_id': ObjectId(id)}):
            if is_hard_delete:
                deleted_obj = q_obj.delete_one({'_id': ObjectId(id)})
            else:
                deleted_obj = await q_obj.update_one(
                    {'_id': ObjectId(id)}, {'$set': {'deleted_at': datetime.now()}}
                )
            if deleted_obj:
                return True
                # return self.convert_data(await q_obj.find_one({'_id': ObjectId(id)}))
